...
public:
    ...
    : m_args(argv)
  {
    ...
    m_display.reset(createDisplay(Size2i(1280, 720)));
...
private:
  void processInput()
  {
    ...
    m_control.use = keys[SDL_SCANCODE_E] || keys[SDL_SCANCODE_SPACE];
  ...
  void draw()
  {
    ...
    if(m_paused)
      m_display->drawText(Vector2f(0, 2), "PAUSE");
    else if(m_slowMotion)
      m_display->drawText(Vector2f(0, 2), "SLOW-MOTION MODE");
    else if(m_control.debug)
      m_display->drawText(Vector2f(0, 2), "DEBUG MODE");

    if(m_textboxDelay > 0)
    {
      m_display->drawText(Vector2f(0, 0), m_textbox.c_str());
      m_textboxDelay--;
    }
